document.addEventListener('DOMContentLoaded', function () {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.watchPosition(function (position) {
                ...
                if (!mapPannedToUserLocation) {
                    // Pan to the user's location only if it's the first time and map is not being panned
                    if (!mapPanning) {
    ...
    // Timer variable for delaying automatic panning after map panning stops
    var panTimer;

    // Event listener for map panning start
    map.on('dragstart', function () {
        mapPanning = true;
    });

    // Event listener for map panning end
    map.on('dragend', function () {
        mapPanning = false;
        // Clear any existing timer to avoid multiple timers running simultaneously
        clearTimeout(panTimer);
        // Set a new timer for automatic panning after 10 seconds
